...
public class LoanService {
    ...
    private boolean assessWhetherTheSalaryIsEqualToOrLessThan3000(BigDecimal income) {
        return income.compareTo(BigDecimal.valueOf(3000)) <= 0;
    }

    private boolean assessWhetherTheSalaryIsBetween3000And5000(BigDecimal income) {
        return income.compareTo(BigDecimal.valueOf(3000)) > 0 && income.compareTo(BigDecimal.valueOf(5000)) <= 0;
    }

    private boolean assessAgeAndLocation(Integer age, String location) {
        return age < 30 && location.equalsIgnoreCase("SP");
    }

    private void assessIfTheSalaryIsEqualOrGreaterThan5000(BigDecimal income, Set<LoanDTO> loans) {
    ...
    private void addPersonalAndGuaranteedLoans(Set<LoanDTO> loans) {
